resource "aws_instance" "roboshop" {
    ...
    tags = merge(
        var.common_tags,
        {
            Name = "${var.instances[count.index]}-${var.environement}"
...
resource "aws_security_group" "allow_all" { #allow all means this is for internal purpose
    name = "${var.sg_name}-${var.environement}" # this is security gruop name , internal name
    description = "${var.sg_dis}-${var.environement}"
    ingress {
    ...
    tags = merge(
        var.common_tags,
        {
            Name = "${var.sg_name}-${var.environement}"
        }
    )
